...
#define X86_MAGIC_HDRS 0x53726448
        ...
        struct setup_header *hdr =
            (struct setup_header *)((char *)kernel_data + X86_REAL_MODE_HEADER_OFFSET);
        ...
        // IOAPIC: GSI (global system inerrupt). I/O Advanced Programmable Interrupt Controller
        // Local APIC: per vCPU APIC
        // IRQチップを作成（PIC, IOAPIC, Local APIC）
        ...
        bp->hdr.loadflags |= (1 << 0); // bzImage の場合には bit0 をセットするらしい。CAN_USE_HEAP (bit7) は不要？

        // i8042 は無効化する。firecracker を参考にした。
        const char *cmdline = "console=ttyS0 audit=0 selinux=0 root=/dev/vda nokaslr i8042.noaux i8042.nomux i8042.dumbkbd";
        ...
        bp->e820_table[3].type = E820_TYPE_RAM;
        ...
        for (;;) {
                ...
                case KVM_EXIT_IO:
                        if (run->io.port >= 0x3f8 && run->io.port <= 0x3ff) {
                                ...
                                // 未処理のIOをログ
                                fprintf(stderr, "[IO %s port=0x%x size=%d]\n",
