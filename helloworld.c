...
int main(void) {
        ...
        if (api_ver != KVM_API_VERSION) {
                fprintf(stderr,
                        "KVM API version mismatch: got=%d expected=%d\n",
                        api_ver, KVM_API_VERSION);
                return 1;
        }

        int vm_fd =
            ioctl(dev_fd, KVM_CREATE_VM, 0); // KVM_X86_DEFAULT_VM = 0 (default)
        if (vm_fd < 0)
        ...
        struct kvm_userspace_memory_region region = {
            ...
            // physical memory slot. Bits 0-15 of “slot” specify the slot id and
            // this value should be less than the maximum number of user memory
            // slots supported per VM. The maximum allowed slots can be queried
            // using KVM_CAP_NR_MEMSLOTS. Slots may not overlap in guest
        ...
        struct kvm_run *run =
            mmap(NULL, (size_t)vcpu_mmap_size, PROT_READ | PROT_WRITE,
                 MAP_SHARED, vcpu_fd, 0);
        if (run == MAP_FAILED)
                die("mmap(kvm_run)");

        struct kvm_sregs sregs;
        check_ioctl(ioctl(vcpu_fd, KVM_GET_SREGS, &sregs),
                    "ioctl(KVM_GET_SREGS)");
        sregs.cs.base = 0;
        sregs.cs.selector = 0;
        ...
        sregs.efer = 0;
        check_ioctl(ioctl(vcpu_fd, KVM_SET_SREGS, &sregs),
                    "ioctl(KVM_SET_SREGS)");
        ...
        for (;;) {
                ...
                case KVM_EXIT_IO: {
                        ...
                        if (run->io.port != com1_port || run->io.size != 1) {
                                fprintf(
                                    stderr,
                                    "Unhandled IO port=0x%x size=%u count=%u\n",
                ...
                case KVM_EXIT_FAIL_ENTRY:
                        fprintf(stderr,
                                ...
                                (unsigned long long)run->fail_entry
